...
if (app && Number(app.dataset.displayId)) {
    ...
    function closedTicketLabel(ticket) {
        if (ticket.close_reason === "completed") return "Done";
        if (ticket.close_reason === "pos_cancelled") return "POS cancelled";
        if (ticket.close_reason === "routing") return "Route closed";
        return "Dismissed";
    }
    ...
    function ticketCard(ticket, payload) {
        ...
        const late = ticket.is_open && minutes >= payload.display.alert_minutes;
        const card = element("article", `ticket-card${late ? " late" : ""}${ticket.has_changes ? " has-update" : ""}${ticket.is_open ? "" : " is-closed"}`);
        ...
        const timerValue = ticket.is_open ? timerStart : ticket.completed_at;
        const timerPrefix = ticket.is_open ? "" : closedTicketLabel(ticket);
        const timerText = timerPrefix
            ? `${timerPrefix} · ${ageLabel(timerValue)}`
            : ageLabel(timerValue);
        const timer = element("div", `ticket-timer${late ? " urgent" : ""}`, timerText);
        timer.dataset.timeValue = timerValue || "";
        timer.dataset.prefix = timerPrefix;
        ...
        if (ticket.is_open && cancelStage && ticket.stage_id !== cancelStage.id) {
        ...
        if (!ticket.is_open) {
            const recall = element("button", "primary-action", "Recall Ticket");
            recall.type = "button";
    ...
    function render(payload) {
        ...
        board.style.setProperty("--stage-count", Math.max(payload.stages.length, 1));
        payload.stages.forEach((stage) => {
            ...
            if (!tickets.length) {
                const empty = element("div", "stage-empty", "No orders");
                stack.append(empty);
            }
            column.append(stack);
            board.append(column);
        ...
        hasRendered = true;
        activeCount.textContent = payload.tickets.filter((ticket) => ticket.is_open).length;
    ...
    setInterval(() => {
        ...
        document.querySelectorAll(".ticket-timer").forEach((timer) => {
            const prefix = timer.dataset.prefix;
            timer.textContent = prefix
                ? `${prefix} · ${ageLabel(timer.dataset.timeValue)}`
                : ageLabel(timer.dataset.timeValue);
            if (!prefix && latestPayload && ageMinutes(timer.dataset.timeValue) >= latestPayload.display.alert_minutes) {
