...
class OhanaKitchenDisplay(models.Model):
    ...
    closed_ticket_minutes = fields.Integer(
        string="Keep Closed Tickets Visible (Minutes)",
        default=20,
        required=True,
        help="Keep completed and cancelled tickets on their stage briefly for recall.",
    )
    ...
    @api.constrains(
        "refresh_seconds",
        "alert_minutes",
        "closed_ticket_minutes",
    )
    def _check_positive_intervals(self):
        for display in self:
            if (
                display.refresh_seconds < 1
                or display.alert_minutes < 1
                or display.closed_ticket_minutes < 1
            ):
    ...
    def ohana_screen_payload(self):
        ...
        closed_cutoff = fields.Datetime.now() - timedelta(
            minutes=self.closed_ticket_minutes
        )
        ...
        return {
            ...
            "display": {
                ...
                "closed_ticket_minutes": self.closed_ticket_minutes,
