...
class OhanaKitchenController(http.Controller):
    ...
    def _check_kitchen_access():
        ...
        if not (
            ...
            or user.has_group("ohana_kitchen_display.group_ohana_kitchen_user")
            or user.has_group("ohana_kitchen_display.group_ohana_kitchen_manager")
        ):
            raise AccessError(_("Ohana Kitchen access is required."))

    @staticmethod
    def _can_configure_kitchen():
        user = request.env.user
        return user.has_group("base.group_system") or user.has_group(
            "ohana_kitchen_display.group_ohana_kitchen_manager"
        )

    @staticmethod
    def _check_kitchen_configuration_access():
        if not OhanaKitchenController._can_configure_kitchen():
            raise AccessError(_("Ohana Kitchen configuration access is required."))
    ...
    def kitchen_screen(self, display_id=None, **kwargs):
        ...
        return request.render(
            "ohana_kitchen_display.kitchen_screen",
            {
                ...
                "displays": displays,
                "user": request.env.user,
                "can_configure": self._can_configure_kitchen(),
            },
        )

    @http.route(
        "/ohana/kitchen/configuration",
        type="http",
        auth="user",
        methods=["GET"],
    )
    def kitchen_configuration(self, **kwargs):
        self._check_kitchen_configuration_access()
        action = request.env.ref(
            "ohana_kitchen_display.action_ohana_kitchen_display"
        )
